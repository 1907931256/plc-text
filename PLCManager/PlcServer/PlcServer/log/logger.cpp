...
void Logger::GenerateLogName()
{
    ...
    sprintf(temp, "%04d-%02d-%02d-%02d-%02d-%02d.txt", pTimeInfo->tm_year+1900, pTimeInfo->tm_mon + 1, pTimeInfo->tm_mday,
...
void Logger::WriteFile(const unsigned char *pData, int nLength)
{
	time_t curTime;
	struct tm * pTimeInfo = NULL;
	time(&curTime);
	...
	char temp[1024] = {0};
	//日志的名称如：2013-01-01.log
	sprintf(temp, "%04d-%02d-%02d-%02d-%02d-%02d-filedata.txt", pTimeInfo->tm_year+1900, pTimeInfo->tm_mon + 1, pTimeInfo->tm_mday,
		pTimeInfo->tm_hour, pTimeInfo->tm_min, pTimeInfo->tm_sec);
	if(m_pFile ==NULL)
	{
		m_pFile = fopen(temp, "a+");
	}
