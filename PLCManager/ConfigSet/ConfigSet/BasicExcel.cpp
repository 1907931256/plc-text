...
psText = new char[dwNum+1];
   if(!psText)
  {
   delete []psText;
    ...
    WideCharToMultiByte (CP_OEMCP,NULL,filename,-1,psText,dwNum,NULL,FALSE);
    locale loc=locale::global(locale(".936"));
	file_.open(psText, ios::in | ios::out | ios::trunc);
	file_.close();
	file_.clear();
...
// PROMISE: Return true if file is successfully opened, false if otherwise.
{
	...
	DWORD dwNum = WideCharToMultiByte(CP_OEMCP,NULL,filename,-1,NULL,0,NULL,FALSE);
	filename_.resize(dwNum+20, 0);
    WideCharToMultiByte (CP_OEMCP,NULL,filename,-1,&*(filename_.begin()),dwNum,NULL,FALSE);
