...
void CClientManager::InitData()
{
	...
	//int enablePriorityInt;
	//if(iniFile.ReadInt("MODEPRIORITY","Enable",enablePriorityInt))
	//{
	//	if(enablePriorityInt==0)
	//		priorityEnable = true;
	//}

	/*std::string landedString;
	if(iniFile.ReadString("Spreader","Landed",landedString))
	{
	std::vector<std::string> LandedAddrVec;
	StringSplit(landedString, ",", LandedAddrVec);
	mSpreaderAddr.landedAddress =  atoi(LandedAddrVec[0].c_str());
	mSpreaderAddr.landeAddressBit = atoi(LandedAddrVec[1].c_str()); 
	std::string lockedString;
	if(iniFile.ReadString("Spreader","Locked",lockedString))
	{
	std::vector<std::string>LockedAddrVec;
	StringSplit(lockedString,",",LockedAddrVec);
	mSpreaderAddr.lockedAddress = atoi(LockedAddrVec[0].c_str());
	mSpreaderAddr.lockedBit = atoi(LockedAddrVec[1].c_str());
	std::string twinModeString;
	if(iniFile.ReadString("Spreader","TwinMode",twinModeString))
	{
	std::vector<std::string>twinModeAddrVec;
	StringSplit(twinModeString,",",twinModeAddrVec);
	mSpreaderAddr.twinModeAddress = atoi(twinModeAddrVec[0].c_str());
	mSpreaderAddr.twinModeBit = atoi(twinModeAddrVec[1].c_str());
	mSpreaderAddr.addressReady = true;
	}
	}
	}
	std::string trolleyHeightAddrString;
	if(iniFile.ReadString("TrolleyHeight","Height",trolleyHeightAddrString))
	{
	std::vector<std::string>trolleyHeightAddrVec;
	StringSplit(trolleyHeightAddrString,",",trolleyHeightAddrVec);
	mTrolleyHeightAddr.heightAddress = atoi(trolleyHeightAddrVec[0].c_str());
	mTrolleyHeightAddr.length = atoi(trolleyHeightAddrVec[1].c_str());
	mTrolleyHeightAddr.addressReady = true;
	}*/
...
//======2017/5/29 OPC
void CClientManager::InitOpcData()
{
	CIniFile iniFile(GetCurrentPath() + "\\config\\General.ini");
	///////////////////////////////////////////////////////////////////////////////
	iniFile.ReadString("OPC_SET","Host",mOpcHost);
	iniFile.ReadString("OPC_SET", "ServerName", mOpcServerName);
	std::string group;
	if(iniFile.ReadString("OPC_SET", "GroupSet", group))
	{
		std::vector<std::string>groupVec;
		StringSplit(group, ",", groupVec);
		for (std::vector<std::string>::iterator ite = groupVec.begin();
			ite != groupVec.end(); ite++)
		{
			std::vector<std::string> groupMsg;
			StringSplit(*ite, ":",groupMsg);
			if(groupMsg.size()==2)
				mOpcGroupMap.insert(std::make_pair(atoi(groupMsg[0].c_str()), groupMsg[1]));
		}
	}
	iniFile.ReadString("OPCITEMSET","HoistPosition",mOpcItemHoistPos);
	iniFile.ReadString("OPCITEMSET","TrolleyPosition",mopcItemTrolleyPos);
	iniFile.ReadString("OPCITEMSET","HoistUp",mOpcHoistUp);
	iniFile.ReadString("OPCITEMSET","HoistDown",mOpcHoistDown);
	iniFile.ReadString("OPCITEMSET","SpreaderLanded",mOpcSpreaderLanded);
	iniFile.ReadString("OPCITEMSET","SpreaderLock",mOpcSpreaderLock);
	iniFile.ReadString("OPCITEMSET","SpreaderUnlock",mOpcSpreaderUnlock);
	iniFile.ReadString("OPCITEMSET","Feet20",mOpcItemFeet.feet20);
	iniFile.ReadString("OPCITEMSET","Feet40",mOpcItemFeet.feet40);
	iniFile.ReadString("OPCITEMSET","Feet45",mOpcItemFeet.feet45);
	iniFile.ReadString("OPCITEMSET","Feet2*20",mOpcItemFeet.feetDou20);
	//初始化分组配置文件
	InitGroupData();
...
void CClientManager::SetIpcState(const char* pIpcStatelist,int group)
{
	m_lockClient.acquire();
	if(group==-1)
	{
	...
	else if(group>0)//从MySql获取ipc状态  2017/4/26
	{
...
void CClientManager::StartGetOpcData()
{
	...
	CClientManager::GetInstance()->createClient("", 0, info, 0, 0, 3, 0);
	m_bGetOpcData = TRUE;
...
{
	m_nOpcHeartBeat = GetSysTimeMicros()/1000;
	while(m_bGetOpcData)
	{
		GetOpcDataInfo();
		Sleep(m_nRefresh);
...
//===2017/5/29 OPC
void CClient::ReadOpcDataProcess() //连接成功的话就定时获取信息
{
	CClientManager* pInstance = CClientManager::GetInstance();
	if (m_bConnectOpc)
	{
...
void CClient::loginOpcServer()
{
	CClientManager* pInstance = CClientManager::GetInstance();
	mOpcHost =COPCClient::init();
	//CString hostName(pInstance->mOpcHost.c_str()) ;
	// COPCClient::init();
	//COPCHost *host = COPCClient::makeHost(hostName);
	//COPCHost *host = COPCClient::init(hostName);
	//List servers
	...
	//}
	// connect to OPC
	CString serverName(pInstance->mOpcServerName.c_str());//"CAEN.HVOPCServer.1";
	mOpcServer = mOpcHost->connectDAServer(serverName);
	ServerStatus status;
	...
	if (m_bConnect)
	{
			...
			ite++)
		{
			...
			if(res >= 0)
			{
				...
				PreserPointCall(iGroup,pBuff,usSwitchOver);
				/////////////////////自由切屏///////////////////////////////////////////
...
void CClient::IpcPtzOperation(int nGroup, const unsigned char *pBuf, int iSwitchGroup)
{
	...
	int iPtzOld = 0;
	int iPtzNew = 0;
	bool commandToCam = TRUE; // send to cctv or cam
	CClientManager* pInstance = CClientManager::GetInstance();
	int iPTZOffset = 0;
		...
		if (bAuto)/*select AUTO*/
		{
			...
			if(commandToCam) //send to cam
			{
				if (iMaskPtz != 8)//
				{
					...
					pInstance->PtzOpration2CAM(nGroup, iIPCIndex,strPTZCmd,iMaskPtz, iSwitchGroup);
				}
				...
				for (int i = 0; i < nHeigthGroups;i ++)
				{
					...
					if (iValue != -1)
					{
						...
						if (itt != m_GroupZoomState.end())
						{
							...
							{
								//if(pInstance->sendCgiCommadZoom(sPuid,iValue,nGroup))//failure
								//{
								//	//zoom操作日志
								//	char szlog[MAX_STR_LEN] = {0};
...
void CClient::PreserPointCall(int nGroup,const unsigned char*pBuff,int iSwitchGroup/*=0*/)
{
	CClientManager* pInstance = CClientManager::GetInstance();
		...
		for(int i=0;i<ipcPresetPointCallOperate.size();i++)
		{
			if(ipcPresetPointCallOperate[i].pointStatuse)
			{
				...
				if(iteIPCInfoMap==pInstance->m_GroupIpcId.end())
					continue;
				std::string ipcIp = iteIPCInfoMap->second.Ip;
				std::string postString = "http://"+ipcIp+"/axis-cgi/com/ptz.cgi?gotoserverpresetname="+ipcPresetPointCallOperate[i].presetPointName;
				if(pInstance->sendCgiCommad(postString,nGroup))
				{
					continue;
				}
	...
	else
	{
		...
		if(!iter->second)//复位
		{
			...
			for(int i=0;i<ipcPresetPointCallOperate.size();i++)
			{
				ipcPresetPointCallOperate[i].pointStatuse = true;
			}
			iter->second = true;
		}
	}
